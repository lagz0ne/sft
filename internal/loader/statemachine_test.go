...
start:
  on:
    click: selecting
`)
	ts, states, _, _, err := ParseStateMachine(*node)
	if err != nil {
		t.Fatal(err)
	}
	if len(ts) != 1 {
...
idle:
  on:
    tick: .
`)
	ts, _, _, _, err := ParseStateMachine(*node)
	if err != nil {
		t.Fatal(err)
	}
	if len(ts) != 1 {
...
beta:
  on:
    back: alpha
`)
	_, states, _, _, err := ParseStateMachine(*node)
	if err != nil {
		t.Fatal(err)
	}
	if states[0] != "alpha" {
...
func TestNotMappingNode(t *testing.T) {
	...
	_, _, _, _, err := ParseStateMachine(*doc.Content[0])
	if err == nil {
